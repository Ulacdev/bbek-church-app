...
CREATE TABLE IF NOT EXISTS `tbl_audit_trail` (
  ...
  `user_name` VARCHAR(255) NULL COMMENT 'User full name',
  `user_position` VARCHAR(100) NULL COMMENT 'User position/role (e.g., admin, pastor, member)',
  `action_type` VARCHAR(50) NOT NULL COMMENT 'Type of action: CREATE, UPDATE, DELETE, VIEW, VIEW_LIST, LOGIN, LOGOUT, EXPORT, VIEW_CERTIFICATE, etc.',
  ...
  `description` TEXT NULL COMMENT 'The API route/endpoint that was accessed (e.g., GET /api/church-records/members/createMember)',
  ...
  `old_values` JSON NULL COMMENT 'Previous values before update (for UPDATE actions) - stored as JSON',
  `new_values` JSON NULL COMMENT 'New values after action (for CREATE/UPDATE actions) - stored as JSON',
