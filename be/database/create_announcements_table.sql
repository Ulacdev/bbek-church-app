...
CREATE TABLE IF NOT EXISTS `tbl_announcements` (
  ...
  `target_audience` VARCHAR(50) NOT NULL DEFAULT 'all' COMMENT 'Target: all, admin, pastor, member, non_member',
  ...
  `updated_at` DATETIME NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT 'Timestamp when announcement was last updated',
...
CREATE TABLE IF NOT EXISTS `tbl_user_announcements` (
  ...
  CONSTRAINT `fk_user_announcement_announcement` 
    ...
    REFERENCES `tbl_announcements` (`announcement_id`) 
    ON DELETE CASCADE
