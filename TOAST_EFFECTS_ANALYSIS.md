...
```vue
const result = await accountsStore.forgotPassword(forgotPasswordForm.email) if
(result) { ElMessage.success('Password reset link has been sent to your email
address') ✓ closeDialog()
```
...
```vue
const requestNewLink = () => { // Navigate to forgot password request page or
shows TODO // TODO: Navigate to forgot password request page or show dialog }
```
