...
const rules = {
  ...
  deceased_name: [
    { required: true, message: "Deceased name is required", trigger: "blur" },
    {
      min: 2,
      max: 100,
      message: "Deceased name must be between 2 and 100 characters",
      trigger: "blur",
    },
  ],
  relationship: [
  ...
  location: [
    { required: true, message: "Location is required", trigger: "blur" },
    {
      min: 3,
      max: 150,
      message: "Location must be between 3 and 150 characters",
      trigger: "blur",
    },
  ],
  status: [
...
const validateStatusWithDateTime = {
  validator: (rule, value, callback) => {
    ...
    const serviceDate =
      formData.service_date || formData.baptism_date || formData.marriage_date;
  ...
  validator: async (rule, value, callback) => {
    ...
    try {
      ...
      if (conflict) {
        callback(
          new Error(
            `Time slot already booked. Try: ${conflict.available_times.join(
              ", "
            )}`
          )
        );
...
const validateBusinessHours = {
  validator: (rule, value, callback) => {
    ...
    if (hour < 6 || hour >= 18) {
      callback(
        new Error("Services must be scheduled between 6:00 AM - 6:00 PM")
      );
      return;
    }
...
const updateStatusFromBaptismDate = () => {
  ...
  if (baptismDate > now) {
    ...
    Math.abs(baptismDate.getTime() - now.getTime()) <
    2 * 60 * 60 * 1000
  ) {
    formData.status = "ongoing";
  } else {
