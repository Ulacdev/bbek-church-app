<template>
  <div class="young-people-list">
    <!-- Image -->
    <div class="list-item">
      <div class="item-label">Image</div>
      <div class="item-preview">
        <el-image
          v-if="youngPeopleData.image"
          :src="youngPeopleData.image"
          fit="cover"
          class="preview-image"
        />
      ...
      <div class="item-action">
        ...
        <span v-if="!youngPeopleData.image" class="text-grey ml-2">No file chosen</span>
      </div>
    </div>
    <el-divider />
    ...
    <div class="list-item">
      ...
      <div class="item-preview">
        <span class="text-bold">{{ youngPeopleData.heroTitle }}</span>
      </div>
      <div class="item-action">
        <el-input
          v-model="youngPeopleData.heroTitle"
          size="small"
    ...
    <div class="list-item">
      ...
      <div class="item-preview">
        <span class="text-grey">{{ youngPeopleData.heroDescription }}</span>
      </div>
      <div class="item-action">
        <el-input
          v-model="youngPeopleData.heroDescription"
          type="textarea"
          :rows="3"
          size="small"
    ...
    <div class="list-item">
      ...
      <div class="item-preview">
        <span class="text-grey">{{ youngPeopleData.description }}</span>
      </div>
      <div class="item-action">
        <el-input
          v-model="youngPeopleData.description"
          type="textarea"
          :rows="2"
          size="small"
    ...
    <div class="list-item">
      ...
      <div class="item-preview">
        <span class="text-bold">{{ youngPeopleData.ourMinistriesTitle }}</span>
      </div>
      <div class="item-action">
        <el-input
          v-model="youngPeopleData.ourMinistriesTitle"
          size="small"
    ...
    <div class="list-item">
      ...
      <div class="item-preview">
        <span class="text-grey">{{ youngPeopleData.ourMinistriesText }}</span>
      </div>
      <div class="item-action">
        <el-input
          v-model="youngPeopleData.ourMinistriesText"
          type="textarea"
          :rows="3"
          size="small"
    ...
    <div class="list-item">
      ...
      <div class="item-preview">
        <span class="text-bold">{{ youngPeopleData.joinCommunityTitle }}</span>
      </div>
      <div class="item-action">
        <el-input
          v-model="youngPeopleData.joinCommunityTitle"
          size="small"
    ...
    <div class="list-item">
      ...
      <div class="item-preview">
        <span class="text-grey">{{ youngPeopleData.joinCommunityText }}</span>
      </div>
      <div class="item-action">
        <el-input
          v-model="youngPeopleData.joinCommunityText"
          type="textarea"
          :rows="3"
          size="small"
    ...
    <div class="list-item">
      ...
      <div class="item-preview">
        <el-button
          :style="{ backgroundColor: youngPeopleData.joinButtonColor, borderColor: youngPeopleData.joinButtonColor }"
          size="small"
          type="primary"
        >
          {{ youngPeopleData.joinButtonText }}
        </el-button>
      </div>
      <div class="item-action">
        <el-input
          v-model="youngPeopleData.joinButtonText"
          size="small"
    ...
    <div class="list-item">
      ...
      <div class="item-preview">
        <div
          class="color-preview"
          :style="{ backgroundColor: youngPeopleData.joinButtonColor }"
        ></div>
      </div>
      <div class="item-action">
        <el-color-picker
          v-model="youngPeopleData.joinButtonColor"
          size="small"
        ></el-color-picker>
      </div>
    </div>
    ...
    <div class="list-item">
      ...
      <div class="item-preview">
        <div
          class="color-preview"
          :style="{ backgroundColor: youngPeopleData.backgroundColor }"
        ></div>
      </div>
      <div class="item-action">
        <el-color-picker
          v-model="youngPeopleData.backgroundColor"
          size="small"
        ></el-color-picker>
      </div>
    </div>
...
const { loading, saving, loadPageData, savePageData } = useCms('youngpeople')

// Default data structure
const defaultYoungPeopleData = {
  image: '',
  heroTitle: 'Young People Department',
...
const createReactiveCopy = (data) => {
  if (!data) return reactive(JSON.parse(JSON.stringify(defaultYoungPeopleData)))
  return reactive(JSON.parse(JSON.stringify(data)))
}

const youngPeopleData = props.youngPeopleData
  ? createReactiveCopy(props.youngPeopleData)
  : reactive(JSON.parse(JSON.stringify(defaultYoungPeopleData)))

// Load data from CMS on mount
onMounted(async () => {
  if (props.activeSection === 'youngPeople') {
    const loadedData = await loadPageData()
    if (loadedData) {
      Object.keys(loadedData).forEach(key => {
        youngPeopleData[key] = loadedData[key]
      })
    }
  }
})
...
watch(() => props.youngPeopleData, (newData) => {
  if (newData) {
    const cloned = JSON.parse(JSON.stringify(newData))
    Object.keys(cloned).forEach(key => {
      youngPeopleData[key] = cloned[key]
    })
  }
}, { deep: true })
...
const handleImageChange = (file) => {
  ...
  reader.onload = (e) => {
    youngPeopleData.image = e.target.result
  }
...
const saveChanges = async () => {
  ...
  try {
    const contentToSave = JSON.parse(JSON.stringify(youngPeopleData))
    const imagesToSave = {}
    ...
    console.error('Error saving young people page:', error)
  }
}
</script>
